...
double Utils::computeInvMass(double Px1,
  ...
  return sqrt((Energy1 + Energy2 + Energy3) * (Energy1 + Energy2 + Energy3) -
              ((Px1 + Px2 + Px3) * (Px1 + Px2 + Px3) + (Py1 + Py2 + Py3) * (Py1 + Py2 + Py3) +
               (Pz1 + Pz2 + Pz3) * (Pz1 + Pz2 + Pz3)));
}

double Utils::computeEta(double Px, double Py, double Pz) {
...
double Utils::computeEtaPhiDistance(double Px1, double Py1, double Pz1, double Px2, double Py2, double Pz2) {
...
void Utils::computeLS(double Vx,
  ...
  *deltaD = sqrt((Vx - Wx) * (Vx - Wx) + (Vy - Wy) * (Vy - Wy) + (Vz - Wz) * (Vz - Wz));
  if (*deltaD > 0.)
    *deltaDErr = sqrt((Vx - Wx) * (Vx - Wx) * VxErr2 + (Vy - Wy) * (Vy - Wy) * VyErr2 + (Vz - Wz) * (Vz - Wz) * VzErr2 +

                      (Vx - Wx) * (Vy - Wy) * 2. * VxyCov + (Vx - Wx) * (Vz - Wz) * 2. * VxzCov +
                      (Vy - Wy) * (Vz - Wz) * 2. * VyzCov +

                      (Vx - Wx) * (Vx - Wx) * WxErr2 + (Vy - Wy) * (Vy - Wy) * WyErr2 + (Vz - Wz) * (Vz - Wz) * WzErr2 +

                      (Vx - Wx) * (Vy - Wy) * 2. * WxyCov + (Vx - Wx) * (Vz - Wz) * 2. * WxzCov +
                      (Vy - Wy) * (Vz - Wz) * 2. * WyzCov) /
                 *deltaD;
  else
    *deltaDErr = 0.;
}
