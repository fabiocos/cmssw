...
B0Isolation::B0Isolation(reco::Vertex bestVtx,
  ...
  for (uint itrkiso = 0; itrkiso < tracks->size(); itrkiso++) {
    ...
    if (itrkiso == itrkm || itrkiso == itrkp)
      continue;
    ...
    for (unsigned int i = 0; i < mum.numberOfSourceCandidatePtrs(); ++i) {
      ...
      if (!((mum.sourceCandidatePtr(i)).isNonnull() && (mum.sourceCandidatePtr(i)).isAvailable()))
        continue;
      const reco::Candidate& cand = *(source);
      if (cand.charge() == 0 || cand.bestTrack() == nullptr)
        continue;
      try {
        cand.bestTrack()->eta();
      } catch (...) {
    ...
    for (unsigned int i = 0; i < mup.numberOfSourceCandidatePtrs(); ++i) {
      ...
      if (!((mup.sourceCandidatePtr(i)).isNonnull() && (mup.sourceCandidatePtr(i)).isAvailable()))
        continue;
      const reco::Candidate& cand = *(source);
      if (cand.charge() == 0 || cand.bestTrack() == nullptr)
        continue;
...
float B0Isolation::LHCbIsolation(ClosestApproachInRPhi ClosestApp,
  ...
  if (ClosestApp.distance() < 0.1) {
    ...
    if (tmpVertexFitTree->isValid() && tmpVertex->vertexIsValid()) {
      ...
      alpha = tmp_lv.Angle(TVector3(
          tmpVertex->position().x() - vtx_x, tmpVertex->position().y() - vtx_y, tmpVertex->position().z() - vtx_z));
