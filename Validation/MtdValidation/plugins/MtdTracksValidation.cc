...
void MtdTracksValidation::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  edm::Handle< reco::TrackCollection > GenRecTrackHandle;
  iEvent.getByToken(GenRecTrackToken_, GenRecTrackHandle);
  edm::Handle< reco::TrackCollection > RecTrackHandle;
  iEvent.getByToken(RecTrackToken_, RecTrackHandle);
  ...
  iEvent.getByToken(tmtdToken_, tMtdH);
  const auto& tMtd = *tMtdH;
  edm::Handle<edm::ValueMap<float>> SigmatMtdH;
  iEvent.getByToken(SigmatmtdToken_, SigmatMtdH);
  const auto& SigmatMtd = *SigmatMtdH;
  edm::Handle<edm::ValueMap<float>> t0SrcH;
  iEvent.getByToken(t0SrcToken_, t0SrcH);
  const auto& t0Src = *t0SrcH;
  edm::Handle<edm::ValueMap<float>> Sigmat0SrcH;
  iEvent.getByToken(Sigmat0SrcToken_, Sigmat0SrcH);
  const auto& Sigmat0Src = *Sigmat0SrcH;
  edm::Handle<edm::ValueMap<float>> t0PidH;
  iEvent.getByToken(t0PidToken_, t0PidH);
  const auto& t0Pid = *t0PidH;
  edm::Handle<edm::ValueMap<float>> Sigmat0PidH;
  iEvent.getByToken(Sigmat0PidToken_, Sigmat0PidH);
  const auto& Sigmat0Pid = *Sigmat0PidH;
  edm::Handle<edm::ValueMap<float>> t0SafeH;
  iEvent.getByToken(t0SafePidToken_, t0SafeH);
  const auto& t0Safe = *t0SafeH;
  ...
  edm::Handle<HepMCProduct> GenEventHandle;
  iEvent.getByToken(HepMCProductToken_, GenEventHandle);
  const HepMC::GenEvent* mc = GenEventHandle->GetEvent();
  double zsim = (*(mc->vertices_begin()))->position().z()/10.;
  double tsim = (*(mc->vertices_begin()))->position().t() * CLHEP::mm / CLHEP::c_light;

  edm::ESHandle<HepPDT::ParticleDataTable> pdTable;
  iSetup.getData(pdTable);
  ...
  edm::Handle< CrossingFrame<PSimHit> > btlSimHitsHandle;
  ...
  for (auto const& simHit : btlSimHits) {
    ...
    (simHitIt->second).energy += simHit.energyLoss()*1000.;
  }

  edm::Handle< CrossingFrame<PSimHit> > etlSimHitsHandle;
  ...
  iEvent.getByToken(trackingParticleCollectionToken_, tpHandle);
  TrackingParticleCollection tpColl = *(tpHandle.product());
  size_t tpindex(0);
  for (auto tp = tpColl.begin(); tp != tpColl.end(); tp++, ++tpindex) {
    TrackingParticleRef tpref(tpHandle, tpindex);
...
const std::pair<bool, bool> MtdTracksValidation::checkAcceptance(const reco::Track& track,
  ...
  edm::ESHandle<MTDGeometry> geomH;
  iSetup.get<MTDDigiGeometryRecord>().get(geomH);
  const MTDGeometry* geom = geomH.product();

  edm::ESHandle<MTDTopology> mtdTopoH;
  iSetup.get<MTDTopologyRcd>().get(mtdTopoH);
  const MTDTopology* topo = mtdTopoH.product();

  edm::ESHandle<MTDDetLayerGeometry> mtdDetLayGeomH;
  iSetup.get<MTDRecoGeometryRecord>().get(mtdDetLayGeomH);
  const MTDDetLayerGeometry* layerGeo = mtdDetLayGeomH.product();

  edm::ESHandle<MagneticField> mfieldH;
  iSetup.get<IdealMagneticFieldRecord>().get(mfieldH);
  ...
  for (const DetLayer* ilay : layersBTL) {
    ...
    for (const auto& detWithState : compDets) {
      ...
      for (const auto& recHit : *btlRecHitsHandle) {
        ...
        DetId geoId = detId.geographicalId(static_cast<BTLDetId::CrysLayout>(topo->getMTDTopologyMode()));
