...
void BtlLocalRecoValidation::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  iEvent.getByToken(btlRecCluToken_, btlRecCluHandle);
  edm::Handle< MTDTrackingDetSetVector > mtdTrkHitHandle;
  iEvent.getByToken(mtdTrackingHitToken_, mtdTrkHitHandle);

  edm::Handle< CrossingFrame<PSimHit> > btlSimHitsHandle;
  iEvent.getByToken(btlSimHitsToken_, btlSimHitsHandle); 
  const auto& btlSimHits = MixCollection<PSimHit>(btlSimHitsHandle.product ());
  ...
  for (const auto& recHit : *btlRecHitsHandle) {
    ...
    if (m_btlSimHits.count(detId.rawId()) == 1 && m_btlSimHits[detId.rawId()].energy > hitMinEnergy_) {
      //float longpos_res = recHit.position() - (m_btlSimHits[detId.rawId()].x)/10.;
      float time_res = recHit.time() - m_btlSimHits[detId.rawId()].time;
      float energy_res = recHit.energy() - m_btlSimHits[detId.rawId()].energy;

      Local3DPoint local_point_sim((m_btlSimHits[detId.rawId()].x)/10.,
                                   (m_btlSimHits[detId.rawId()].y)/10.,
                                   (m_btlSimHits[detId.rawId()].z)/10.);
      local_point_sim =
    ...
    for (const auto& cluster : DetSetClu) {
      ...
      for (int ihit = 0; ihit < cluster.size(); ++ihit) {
        ...
        for (const auto& recHit : *btlRecHitsHandle) {
          ...
          Local3DPoint local_point_sim((m_btlSimHits[recHit.id().rawId()].x)/10.,
                                       (m_btlSimHits[recHit.id().rawId()].y)/10.,
                                       (m_btlSimHits[recHit.id().rawId()].z)/10.);
          local_point_sim =
