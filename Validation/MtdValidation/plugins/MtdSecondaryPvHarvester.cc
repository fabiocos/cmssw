...
class MtdSecondaryPvHarvester : public DQMEDHarvester {
public:
  explicit MtdSecondaryPvHarvester(const edm::ParameterSet& iConfig);
  ~MtdSecondaryPvHarvester() override;
...
MtdSecondaryPvHarvester::MtdSecondaryPvHarvester(const edm::ParameterSet& iConfig)
    : folder_(iConfig.getParameter<std::string>("folder")) {}

MtdSecondaryPvHarvester::~MtdSecondaryPvHarvester() {}

// auxiliary method to compute efficiency from the ratio of two 1D MonitorElement
void MtdSecondaryPvHarvester::computeEfficiency1D(MonitorElement* num, MonitorElement* den, MonitorElement* result) {
...
void MtdSecondaryPvHarvester::incrementME(MonitorElement* base, MonitorElement* toBeAdded) {
...
// ------------ endjob tasks ----------------------------
void MtdSecondaryPvHarvester::dqmEndJob(DQMStore::IBooker& ibook, DQMStore::IGetter& igetter) {
  ...
  if (!meBarrelPIDp || !meEndcapPIDp || !meBarrelTruePiNoPID || !meBarrelTrueKNoPID || !meBarrelTruePNoPID ||
      ...
      !meEndcapTruePAsP) {
    edm::LogWarning("MtdSecondaryPvHarvester") << "PID Monitoring histograms not found!" << std::endl;
    return;
  }
...
void MtdSecondaryPvHarvester::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;

  desc.add<std::string>("folder", "MTD/SecondaryV/");

  descriptions.add("MtdSecondaryPvPostProcessor", desc);
}

DEFINE_FWK_MODULE(MtdSecondaryPvHarvester);
