...
public:
  ...
  std::vector<int> cluster(const std::vector<std::vector<double>>& embeddings, const std::vector<float>& beta);
...
private:
  ...
  bool within_per_axis_box_(const std::vector<double>& a, const std::vector<double>& b, const size_t D_input) const {
    if (use_dims_.empty()) {
    ...
    for (size_t j = 0; j < use_dims_.size(); ++j) {
      ...
      if (d < 0 || static_cast<size_t>(d) >= D_input)
        continue;
    ...
    return require_all_dims_ ? (n_within == static_cast<int>(use_dims_.size())) : (n_within >= 1);
  }
...
std::vector<int> ObjectCondensationClustering::cluster(
  ...
  for (size_t i = 0; i < N; ++i) {
    if (beta[i] > t_beta_)
      seed_indices.push_back(static_cast<int>(i));
  }
  ...
  for (const auto& kv : cluster_counts) {
    if (kv.second >= min_cluster_size_)
