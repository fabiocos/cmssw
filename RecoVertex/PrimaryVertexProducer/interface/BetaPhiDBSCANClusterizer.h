...
public:
  ...
  std::vector<int> cluster(const std::vector<std::vector<double>>& embeddings,
    ...
    std::sort(seeds.begin(), seeds.end(), [&](int a, int b) { return beta[a] > beta[b]; });
    ...
    for (int seed_idx : seeds) {
      ...
      while (!q.empty()) {
        ...
        neighbors.erase(std::remove_if(neighbors.begin(), neighbors.end(), [&](int nb) { return seed_set.count(nb); }),
                        neighbors.end());
        std::cout << " Neighbours size " << neighbors.size() << std::endl;

        if ((int)neighbors.size() < min_cluster_size)
          continue;
...
private:
  ...
  std::vector<int> get_adaptive_neighbors(int idx,
                                          const std::vector<std::vector<double>>& x,
                                          const std::vector<float>& phi,
                                          const std::vector<float>& beta,
                                          const std::unordered_set<int>& seed_set) const {
