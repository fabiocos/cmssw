...
using namespace std;
enum class ClusterAlgo { Object, DBSCAN, BetaDBSCAN, OCScore };
std::unordered_map<std::string, ClusterAlgo> algoMap = {{"ObjectCond", ClusterAlgo::Object},
...
std::vector<TransientVertex> GNNClusterizer::vertices(const std::vector<reco::TransientTrack>& tracks) const {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    for (int c = 0; c < 3; ++c) {
      float val = (c < embedding_dim) ? static_cast<float>(proj[c]) : std::numeric_limits<float>::quiet_NaN();
