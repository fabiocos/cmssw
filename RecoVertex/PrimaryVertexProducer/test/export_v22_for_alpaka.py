#!/usr/bin/env python3
"""
export_v22_for_alpaka.py - Export v22 model with Alpaka SoA-compatible shapes

Same logic as export_v17p1_for_alpaka.py but pointing to v22 model/checkpoint.
...
Usage:
    ...
    python export_v22_for_alpaka.py
"""
...
class VertexSlotModelCMSSW(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        # v22 VertexSlotModel returns 7 outputs: A, z_hat, t_hat, p, pi, pi_logits, s
...
def main():
    # Paths - pointing to v22
    ckpt_dir = "/afs/cern.ch/work/p/prsolank/private/OC_FastGraph/checkpoints_vertex_slot_v22"
    model_src = "/afs/cern.ch/work/p/prsolank/private/OC_FastGraph/vertex_slot_model_v22.py"
    ckpt_path = os.path.join(ckpt_dir, "best_vertex_slot.pt")
    scaler_path = os.path.join(ckpt_dir, "scaler_stats.pt")
    output_path = "vertex_slot_v22_alpaka.pt"
    
    print("=" * 70)
    print("Export v22 for Alpaka SoA (following train script logic)")
    print("=" * 70)
    
    # Load model definition
    print(f"\n1. Loading model definition from {model_src}")
    model_module = load_module_from_path("vertex_slot_model_v22", model_src)
    ...
    # Create model with EXACT same args as train script
    ...
    # Copy to production
    print(f"\n9. Copying to CMSSW data directory...")
    import shutil
    dest = "../data/vertex_slot_production.pt"
