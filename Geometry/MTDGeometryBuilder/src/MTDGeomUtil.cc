...
void MTDGeomUtil::setGeometry(const MTDGeometry* geom) {
  geom_ = geom;
}

void MTDGeomUtil::setTopology(const MTDTopology* topo) {
  topology_ = topo;
}
...
float MTDGeomUtil::getPt(const DetId& id, const LocalPoint& local_point, const float& hitEnergy, const float& vertex_z) const {
  GlobalPoint position = getPosition(id, local_point);
  float eta = getEta(position, vertex_z);
  float pt = hitEnergy / cosh(eta);
  return pt;
